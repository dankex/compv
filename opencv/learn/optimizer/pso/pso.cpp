...
void init_comm_random(int *comm, pso_settings_t * settings) {
	...
	for (k=0; k<settings->nhood_size; k++) {
	    ...
	    j = gsl_rng_uniform_int((const gsl_rng*) settings->rng, settings->size);
	       ...
	       pso_result_t *solution, pso_settings_t *settings)
{
    ...
    void (*inform_fun)(int *, double *,
 		   double *, double *,
 		   double *, int,
 		   pso_settings_t *); // neighborhood update function
